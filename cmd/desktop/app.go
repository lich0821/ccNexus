...
func (a *App) AddEndpoint(name, apiUrl, apiKey, transformer, model, remark, modelRedirectsJSON string) error {
	var modelRedirects map[string]string
	if modelRedirectsJSON != "" {
		if err := json.Unmarshal([]byte(modelRedirectsJSON), &modelRedirects); err != nil {
			return fmt.Errorf("invalid modelRedirects JSON: %w", err)
		}
	}
	return a.endpoint.AddEndpoint(name, apiUrl, apiKey, transformer, model, remark, modelRedirects)
}
func (a *App) RemoveEndpoint(index int) error { return a.endpoint.RemoveEndpoint(index) }
func (a *App) UpdateEndpoint(index int, name, apiUrl, apiKey, transformer, model, remark, modelRedirectsJSON string) error {
	var modelRedirects map[string]string
	if modelRedirectsJSON != "" {
		if err := json.Unmarshal([]byte(modelRedirectsJSON), &modelRedirects); err != nil {
			return fmt.Errorf("invalid modelRedirects JSON: %w", err)
	...
	return a.endpoint.UpdateEndpoint(index, name, apiUrl, apiKey, transformer, model, remark, modelRedirects)
}
